...
slider.noUiSlider.on('set', function(){
  ...
  ndmc_wms=L.tileLayer.wms( "http://ndmc-001.unl.edu:8080/cgi-bin/mapserv.exe",{
  ...
  map.addLayer(groupedOverlays["Reference Layers"]["Current USDM"])
  groupedOverlays["Reference Layers"]["Current USDM"].addTo(map);
  //map._onResize();
    // w=window.open('text.txt')
    // w.document.write("usdm"+dateString)
    // w.print()
    //delete(ndmc_wms)
});
...
function julianIntToDate(n) {
    ...
    var M = f + 3 - 12 - Math.ceil(f/10);
    var Y = (100*b) + d - 4800 + Math.ceil(f/10);
...
function getWMSdates() {
  ...
  var tdw = julianIntToDate(ldw-4)
  tdw=tdw.getFullYear().toString()+pad((tdw.getMonth()+1).toString(),2)+pad(tdw.getDate().toString(),2)

  //var date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
  // w=window.open('text.txt')
  // w.document.write(tdw)
  // w.print()
...
date=wmsDates[0];
dateString=wmsDates[1];
groupedOverlays["Reference Layers"]["Current USDM"]=L.tileLayer.wms( "http://ndmc-001.unl.edu:8080/cgi-bin/mapserv.exe",{
    map: "/ms4w/apps/usdm/service/usdm_"+date+"_wms.map",
    layers: "usdm"+dateString,
    styles: "default",
    format: "image/png",
